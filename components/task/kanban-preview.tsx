  ...
  return (
    <div className="grid gap-3 overflow-x-auto pb-2 md:grid-cols-2 xl:grid-cols-4">
      {kanbanColumns.map((column) => {
        ...
        return (
          <section key={column} className="min-h-64 rounded-lg border border-slate-200 bg-slate-50">
            <div className="flex items-center justify-between border-b border-slate-200 px-3 py-3">
              <h3 className="text-sm font-extrabold text-slate-800">{column}</h3>
            ...
            <div className="space-y-3 p-3">
              {cards.length === 0 ? (
                ...
                cards.map((task) => (
                  <article key={task.id} className="rounded-md border border-slate-200 bg-white p-3 shadow-sm">
                    <div className="mb-2 flex items-center justify-between gap-2">
                      <Badge variant={task.isDelayed ? "destructive" : "outline"}>{task.status}</Badge>
                      <span className="text-xs text-slate-500">{task.dueDate}</span>
                    </div>
                    <h4 className="line-clamp-2 text-sm font-bold text-slate-950">{task.title}</h4>
